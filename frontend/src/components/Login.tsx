    ...
    return (
        ...
        <div className='min-h-screen flex items-center justify-center'>
            <form
                ...
                <button type="submit" className="mt-2 w-full h-11 rounded-full text-white bg-purple-600 hover:bg-purple-500 transition " >
                    {state === "login" ? "Login" : "Sign up"}
                </button>

                <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="text-gray-400 text-sm mt-3 mb-11 cursor-pointer" >
                    {state === "login" ? "Don't have an account?" : "Already have an account?"}
                    <span className="text-purple-400 hover:underline ml-1">click here</span>
                </p>
            </form>
            </div>
